    ...
    <AdminLayout>
      <div>
        ...
        <div className="flex items-center bg-gray-100 rounded-lg p-3 mb-6">
          ...
          <input type="text" placeholder="Search orders" className="bg-transparent outline-none flex-1" />
        </div>
        ...
        <div className="bg-white rounded-xl shadow">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/orders/${order.id}`)}>
                  <td className="p-4">#{order.id}</td>
                  <td>{order.date}</td>
                  <td>{order.customer}</td>
                  <td><span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs">{order.payment}</span></td>
                  <td><span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs">{order.status}</span></td>
                  <td>{order.total}</td>
                </tr>
              ))}
            </tbody>
